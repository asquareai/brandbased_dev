...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function bbExtractGuidancePadLRpx(rulesText) {
    ...
    /* Applied as margin-left/right on slots (px). Keep a generous cap for admin tuning. */
    const cap = (n) => Math.max(0, Math.min(200, n));
    const mL = t.match(
      /(?:^|[\n;]|\s)(?:add\s+)?padding[-\s]left\s*[:：]?\s*([0-9]{1,4})(?:\s*px)?\b/i
    );
    const mR = t.match(
      /(?:^|[\n;]|\s)(?:add\s+)?padding[-\s]right\s*[:：]?\s*([0-9]{1,4})(?:\s*px)?\b/i
    );
    if (mL) {
      const n = Number(mL[1]);
    ...
    if (out.l == null && out.r == null) {
      let g =
        t.match(/\bpadding\s+([0-9]{1,4})\s*px\b/i) ||
        t.match(/\bpadding\s*:\s*([0-9]{1,4})\s*px\b/i);
      if (g) {
  ...
  /** Layout-hint summary for Team Console (parsed in-browser; works without OpenAI / smart-size server). */
  function bbGuidanceLayoutStatusLine(rulesText) {
    const t = String(rulesText ?? "");
    const pr = bbExtractGuidancePadLRpx(t);
    const padB = bbExtractUserOffsetYpx(t);
    const oy = bbExtractUserOffsetYem(t);
    const lr = [];
    if (pr.l != null) lr.push(`left ${pr.l}px`);
    if (pr.r != null) lr.push(`right ${pr.r}px`);
    let s = `Parsed padding-bottom: ${padB}px, offset-y: ${oy}em`;
    if (lr.length) s += ` · slot margin ${lr.join(", ")}`;
    return s;
  }
  ...
  function bbScheduleGuidanceCommit(rulesText, floatStatus) {
    ...
    _bbGuidanceDebounce = setTimeout(() => {
      try {
        ...
        if (floatStatus) {
          floatStatus.textContent = bbGuidanceLayoutStatusLine(rulesText);
        }
      } catch (e) {
  ...
  function bbInitSmartRulesUi() {
    ...
    let loadedRules = bbGetSmartRules();
    if (!String(loadedRules || "").trim()) {
      const tmpl = document.getElementById("bbTeamConsoleDefaultSmartRules");
      if (tmpl && tmpl.textContent) {
        const t = String(tmpl.textContent || "").trim();
        if (t) loadedRules = t;
      }
    }
    input.value = loadedRules;
    if (status) status.textContent = bbGuidanceLayoutStatusLine(input.value);
    ...
    save.addEventListener("click", () => {
      ...
      if (status) {
        status.textContent = bbGuidanceLayoutStatusLine(rules);
      }
      const placeSt = document.getElementById("bbPlaceGuidanceStatus");
    ...
    clear.addEventListener("click", () => {
      ...
      if (status) status.textContent = bbGuidanceLayoutStatusLine("");
      const placeSt = document.getElementById("bbPlaceGuidanceStatus");
  ...
  /** Intentionally no-op: popup chrome is owned by bb-theme-console-sync (revert of class-stripping hack). */
  function bbEnsurePopupUploadBrandSurfaces(_popup) {}
  ...
  function bbInitAssetLabUi() {
    ...
    function bbCloseCropper() {
      ...
      cropBaseFittedH = 0;
      try {
        document.dispatchEvent(new CustomEvent("bb-asset-lab-crop-closed", { bubbles: true }));
      } catch {}
    }
    ...
    function bbApplyUploadedAssetToMarks(opts) {
      ...
      // Page refresh with saved crop: keep smart-size cache; let enhanceBrandMarks apply sizing.
  ...
  /** Team Console / other tabs: pull latest rules + re-apply L/R hints (same origin only). */
  let _bbGuidanceExternalSyncTimer = null;
  function bbSyncGuidanceFromOtherContexts() {
    clearTimeout(_bbGuidanceExternalSyncTimer);
    _bbGuidanceExternalSyncTimer = setTimeout(() => {
      try {
        bbSyncGuidanceTextareas(bbGetSmartRules());
        if (bbIsBrandAiSmartOn()) bbApplyGuidanceLrPadFromText();
      } catch {}
    }, 80);
  }
  try {
    window.addEventListener("focus", bbSyncGuidanceFromOtherContexts);
    document.addEventListener("visibilitychange", () => {
      if (document.visibilityState === "visible") bbSyncGuidanceFromOtherContexts();
    });
    window.addEventListener("storage", (e) => {
      if (e && e.key === BB_SMART_RULES_KEY) bbSyncGuidanceFromOtherContexts();
    });
  } catch {}
  ...
  function bbShowBrandSettingsThemeHint(anchorEl) {
    ...
    wrap.textContent = "Click Theme Styles to enable popup (next step)";
  ...
  function openModalForBrandMark(el) {
    ...
    const popup =
      ...
      createPopup(el);
    if (!popup) return;
    const otherOpen = document.querySelector(".popup.popup-visible");
    if (otherOpen && otherOpen !== popup) return;
    ...
    // Close “X” + sheet close appear after ~2s (same as Premium / historical Brand Theme backup —
    // docked #bbThemePopupMount preview uses the same staged reveal so the B / loader beat finishes first).
    const closeChromeDelayMs = 2000;
    try {
      ...
      popup.__bbCloseTimer = window.setTimeout(() => {
        try {
          ...
          if (lockIcon) lockIcon.style.marginRight = window.innerWidth <= 980 ? "0px" : "30px";
  ...
  try {
    window.bbOpenModalForBrandMark = openModalForBrandMark;
  } catch {}
  ...
  const BB_STANDALONE_APPEARANCE_KEY = "bbStandaloneAppearance:v1";
  ...
  function bbGetGuidancePadTargetMarks() {
    ...
    for (const el of bbGetStandaloneInTextMarks()) add(el);
    document
  ...
  /** Brand Settings preview + Theme simulated marks (persist placement from sliders / storage). */
  ...
  function stripBrandEnhancements() {
    ...
    try {
      document.querySelectorAll("body > .slider-close-up").forEach((node) => node.remove());
    } catch {}
    try {
  ...
  popup.innerHTML = `
    ...
    <button type="button" class="popup-sheet-close" aria-label="Close popup">
      ...
      <div class="product-element">
        ...
        <div class="swiper swiper-thumb">
          <div class="swiper-wrapper">
            <div class="swiper-slide"><img class="bb-product-placeholder-img" src="BB-Product-Image-Placeholder.svg" alt="" decoding="async" /></div>
          </div>
          <div class="swiper-pagination"></div>
        </div>

        
        <!-- Markup/order matches ui-demo-backup bb-smart-ui createPopup -->
  ...
  const token = brandSlug ? bbBrandClass(brandSlug) : "";
  ...
  /*** INIT SWIPERS FOR THIS POPUP (required for thumb bullets + close-up gallery) ***/
  const thumbSwiperEl = popup.querySelector(".swiper-thumb");
  const closeUpSwiperMountEl = popup.querySelector(".slider-close-up-swiper");
  let thumbSwiper = null;
  let sliderCloseUpSwiper = null;
  if (typeof Swiper !== "undefined") {
    ...
    if (closeUpSwiperMountEl) {
      sliderCloseUpSwiper = new Swiper(closeUpSwiperMountEl, {
        slidesPerView: 1,
        spaceBetween: 10,
        navigation: {
  ...
  const closeUpSwiperEl = sliderCloseUp?.querySelector(".slider-close-up-swiper") || null;
  const closeBtn = sliderCloseUp?.querySelector(".close") || null;
  /* Premium: viewport-ish inset on wide screens; tiny screens rely on bb-shared-ui.css. Brand Theme: always pin X to the swiper card (same visual as Premium-BB) — bb-shared uses !important on .close, so we set inline !important after layout. */
  ...
  const bbDismissCloseUp = () => {
    ...
    if (sliderCloseUp.style.display !== "flex") return;
    try {
      closeUpSwiperEl?.classList.remove("bb-close-up-entrance");
    } catch {}
    sliderCloseUp.style.display = "none";
  };

  const bbRunCloseUpEntrance = () => {
    const root = closeUpSwiperEl;
    if (!root) return;
    try {
      root.classList.remove("bb-close-up-entrance");
      void root.offsetWidth;
      root.classList.add("bb-close-up-entrance");
      window.setTimeout(() => {
        try {
          root.classList.remove("bb-close-up-entrance");
        } catch {}
  ...
  if (sliderCloseUpSwiper) {
    sliderCloseUpSwiper.on("slideChange", () => {
      ...
      requestAnimationFrame(() => bbPositionCloseUpX());
    });
  }

  // Close close-up view
  if (closeBtn) closeBtn.onclick = () => bbDismissCloseUp();
  ...
  try {
    const mo = new MutationObserver(() => {
      if (sliderCloseUp && sliderCloseUp.style.display === "flex") {
        bbPositionCloseUpX();
      }
    });
  ...
  const zoomBtns = Array.from(sliderCloseUp?.querySelectorAll(".zoom-toggle-btn") || []).filter(Boolean);
  const closeUpEl = sliderCloseUp;
  ...
  /** Brand Theme Settings dock: full-screen gallery is disabled; show glass pill (styled in theme-design.css). */
  const bbShowThemeDockThumbZoomHint = (rootPopup) => {
    const productEl = rootPopup.querySelector(".product-element");
    if (!productEl) return;
    let toast = productEl.querySelector(".bb-theme-preview-zoom-toast");
    if (!toast) {
      toast = document.createElement("div");
      toast.className = "bb-theme-preview-zoom-toast";
      toast.setAttribute("role", "status");
      productEl.appendChild(toast);
    }
    toast.textContent = "Zoom gallery opens on your live popup";
    try {
      clearTimeout(productEl._bbThumbZoomToastTimer);
    } catch {}
    requestAnimationFrame(() => {
      try {
    ...
    productEl._bbThumbZoomToastTimer = setTimeout(() => {
      try {
  ...
  popup.querySelectorAll(".swiper-thumb .swiper-slide").forEach((slide, index) => {
    slide.addEventListener("click", (e) => {
      if (!sliderCloseUp || !sliderCloseUpSwiper) return;
      /* Brand Theme dock preview: no close-up sheet — pill hint + thumb strip still advances. */
      if (popup.closest("#bbThemePopupMount")) {
        bbShowThemeDockThumbZoomHint(popup);
        try {
  ...
  try {
    const closeUpSwiperRoot = closeUpSwiperEl;
  ...
  if (closeUpEl) {
    ...
    /* Desktop (>=980): index-brandbased.html — expand modal when zoom gallery opens. Docked Theme uses vh (column layout), not % of viewport width. */
    try {
      const sliderCloseUpEl = sliderCloseUp;
      const closeBtnCloseUp = closeBtn;

      const clearCloseUpExpand = () => {
        popup.style.removeProperty("min-width");
        popup.style.removeProperty("height");
        popup.style.removeProperty("max-height");
      };

      const closeUpDisplayed = () => {
        try {
          if (!sliderCloseUpEl) return false;
          if (sliderCloseUpEl.style.display === "flex") return true;
          return window.getComputedStyle(sliderCloseUpEl).display === "flex";
        } catch {
          return sliderCloseUpEl?.style.display === "flex";
        }
      };

      const flashCloseUpX = () => {
        if (!closeBtnCloseUp) return;
        try {
          closeBtnCloseUp.style.transition = "none";
          closeBtnCloseUp.style.opacity = "0";
          void closeBtnCloseUp.offsetWidth;
          closeBtnCloseUp.style.transition = "opacity 1s ease";
          closeBtnCloseUp.style.opacity = "1";
        } catch {}
      };

      const applyCloseUpExpand = () => {
        ...
        const docked = !!popup.closest("#bbThemePopupMount");
        if (docked) {
          /* Theme CSS uses `height: auto !important` on the docked modal — override with inline !important like index-brandbased expands. */
          popup.style.removeProperty("min-width");
          try {
            popup.style.setProperty("height", "80vh", "important");
      ...
      if (sliderCloseUpEl) {
        const moCu = new MutationObserver((mutations) => {
        ...
        moCu.observe(sliderCloseUpEl, { attributes: true, attributeFilter: ["style"] });
      }
      ...
      window.addEventListener(
        ...
        () => {
          ...
          if (docked) {
            popup.style.removeProperty("min-width");
            try {
              popup.style.setProperty("height", "78vh", "important");
              popup.style.setProperty("max-height", "85vh", "important");
            } catch {
...
const positionPopup = () => {
  ...
  if (window.innerWidth < 840) {
    ...
    // Mobile: swipe down anywhere on the popup to close (requires touch input).
    // We listen at document capture so Swiper/touch handlers can’t swallow the gesture.
    const popupHandle = popup.querySelector('.popup-handle');
    const bbClosePopupNow = () => {
      bbDismissCloseUp();
      popup.classList.remove('popup-visible');
      popup.classList.remove('close-visible');
      popup.style.opacity = '0';
    ...
    popup.querySelector('.popup-sheet-close')?.addEventListener('click', bbClosePopupNow);

    popupHandle?.addEventListener('click', bbClosePopupNow);

    try {
      if (!document.__bbSwipeCloseAttached) {
        document.__bbSwipeCloseAttached = true;

        let active = null; // { popup, x0, y0, x1, y1, canceled }

        const getVisiblePopup = () => document.querySelector('.popup.popup-visible');
        const isCloseUpOpen = () => {
          try {
            if (!sliderCloseUp) return false;
            return window.getComputedStyle(sliderCloseUp).display !== "none";
          } catch {
            return false;
          }
        };
        ...
        document.addEventListener(
          'touchstart',
          (e) => {
            const p = getVisiblePopup();
            if (!p) return;
            if (isCloseUpOpen()) return;
            const t = e.target;
            if (!t || !(t instanceof Element) || !t.closest('.popup.popup-visible')) return;
            const touch = e.touches && e.touches[0];
            if (!touch) return;
            active = { popup: p, x0: touch.clientX, y0: touch.clientY, x1: touch.clientX, y1: touch.clientY, canceled: false };
          },
        ...
        document.addEventListener(
          ...
          (e) => {
            ...
            const touch = e.touches && e.touches[0];
            if (!touch) return;
            active.x1 = touch.clientX;
            active.y1 = touch.clientY;
            const dx = active.x1 - active.x0;
            const dy = active.y1 - active.y0;
            if (Math.abs(dx) > Math.abs(dy) && Math.abs(dx) > 18) active.canceled = true;
          },
          { capture: true, passive: true }
        );
        ...
        document.addEventListener(
          ...
          (e) => {
            ...
            const dx = active.x1 - active.x0;
            const dy = active.y1 - active.y0;
            const p = active.popup;
            active = null;

            if (!p || !p.classList.contains('popup-visible')) return;
            if (isCloseUpOpen()) return;

            if (dy > 85 && Math.abs(dx) < 45) {
              try { e.preventDefault(); } catch {}
              try {
                if (p === popup) bbDismissCloseUp();
              } catch {}
              p.classList.remove('popup-visible');
              p.classList.remove('close-visible');
              p.style.opacity = '0';
              p.style.pointerEvents = 'none';
              try { window.clearTimeout(p.__bbCloseTimer); } catch {}
            }
          },
    ...
    const bbCanSwipeClosePopup = () => {
      ...
      if (sliderCloseUp && sliderCloseUp.style.display === "flex") return false;
      return true;
    };
