  ...
  function bbIsStandaloneDemoPage() {
    try {
      return !!document.querySelector(".bb-premium-wrap, .bb-freemium-wrap");
    } catch (_e) {
      return false;
    }
  }

  function bbStandaloneMarksRevealed() {
    return !!globalThis.__bbStandaloneMarksRevealed;
  }

  function bbSetStandaloneMarksRevealed() {
    globalThis.__bbStandaloneMarksRevealed = true;
  }

  function bbPulseInTextUploadMarks() {
    if (bbIsStandaloneDemoPage() && !bbStandaloneMarksRevealed()) return;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  /** Cross-page fallback when typography context differs (Brand Settings vs standalone). */
  const BB_SMART_LAST_APPLIED_PREFIX = "bbSmartSize:lastApplied:v1:";
  ...
  function bbGetReplaceThreshold() {
    try {
      // Slider exists only on Brand Settings; other pages must not clamp from stale localStorage.
      if (!document.getElementById("bbReplaceThreshold")) return 9999;
  ...
  function bbHasAppearanceAwareLogos() {
    try {
      const light = localStorage.getItem(BB_LU_LIGHT_LOGO_KEY) || "";
      const dark = localStorage.getItem(BB_LU_DARK_LOGO_KEY) || "";
      if (light || dark) return true;
      const svg = localStorage.getItem(BB_ASSET_LAB_STORAGE_KEY) || "";
      return !!(svg && /<\s*svg/i.test(svg));
    } catch (_e) {
      return false;
    }
  }

  function bbGetPopupUploadLogoTargets(popup) {
    const root = popup || document;
    return Array.from(
      root.querySelectorAll(
        ".popup .title-row .brand-icon, .popup .lock-icon-bb-logo, .bb-freemium-badge, button.bb-freemium-badge"
      )
    );
  }

  /** Popup brand/lock marks: set --bb-logo for light/dark uploads (logo blend reads this). */
  function bbEnsurePopupUploadBrandSurfaces(popup) {
    const root = popup || document.querySelector(".popup");
    if (!root) return;
    ...
    let url = scheme === "dark" ? bbLuDarkBlobUrl : bbLuLightBlobUrl;
    let ratio = scheme === "dark" ? bbLuDarkRatio : bbLuLightRatio;
    if (!url) {
      const raw = scheme === "dark" ? darkRaw || lightRaw : lightRaw || darkRaw;
    ...
    bbGetPopupUploadLogoTargets(root).forEach((el) => {
      const token = bbBrandClass(el.dataset?.brand || "upload");
      if (token) el.classList.add(token);
      if (el.classList.contains("bb-upload") || el.dataset.brand === "upload") {
        paint(el);
      }
    });
  }
  ...
  function bbInitAssetLabUi() {
    ...
    async function bbSaveCrop() {
      // Base viewBox must always come from the *current* SVG string (not cropVb from a previous open / other file).
      const svgText = bbAssetLabSvgText || (localStorage.getItem(BB_ASSET_LAB_STORAGE_KEY) || "");
      if (!svgText) {
        setStatus("Upload an SVG first.");
        return;
      ...
      // Sliders: source of truth (Konva rect is a guide).
      const fracs = bbGetCropSliderFracs();
      const newVb = bbVbForCropFracs(baseVb, fracs);
      if (!newVb) return;
      const outSvg = bbSetRootViewBoxHard(svgText, newVb);
      // Verify the *actual* viewBox in the output string (if this fails, the file won't visually crop).
      const vbActual = bbGetRootViewBoxObject(outSvg);
      if (!bbVbCloseEnough(vbActual, newVb, 0.75)) {
        setStatus(
          `ERROR: viewBox did not apply to SVG text (parser mismatch). ` +
            `Wanted: ${bbFmtVb4(newVb)}. Found: ${vbActual ? bbFmtVb4(vbActual) : "NONE"}. ` +
            `If you can, paste the first ~30 lines of the SVG (or the file) so we can match its exporter format.`
        );
        bbCloseCropper();
        return;
      }
      // Required for <img> / CSS: same normalization the cropper uses, or browsers keep the old viewport.
      ...
      const vb = vbAfterNorm ? `${vbAfterNorm.x} ${vbAfterNorm.y} ${vbAfterNorm.w} ${vbAfterNorm.h}` : `${newVb.x} ${newVb.y} ${newVb.w} ${newVb.h}`;
      ...
      if (!appliedToMarks && panel) panel.classList.remove("open");
      // Save the CROPPED SVG to disk immediately (processed-content folder).
      const marksNote = appliedToMarks ? " Logos in the text updated." : " No in-text logo marks on this page (crop is saved).";
      try {
        const originalName = (localStorage.getItem(BB_ASSET_LAB_FILENAME_KEY) || "upload.svg");
        const ping = await fetch(bbPingApi()).catch(() => null);
        if (!ping || !ping.ok) {
          // Do not block saving — ping is only a health check. If CORS/GET is misconfigured, POST may still work.
        }
        const res = await fetch(BB_SAVE_CROPPED_API, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            filename: originalName,
            svg: finalSvg,
            // Prefer numeric viewBox (server tolerates number formatting; avoids brittle substring matches)
            viewBoxX: newVb.x,
            viewBoxY: newVb.y,
            viewBoxW: newVb.w,
            viewBoxH: newVb.h,
            // Optional debug: keep as string for humans/back-compat
            expectedViewBox: vb,
          }),
        });
        if (!res.ok) {
          ...
          setStatus(
            `Crop saved locally (viewBox updated), but server save failed: ${res.status}. ${msg}${marksNote}`
          );
        } else {
          const j = await res.json();
          const savedVb = j && j.viewBox ? String(j.viewBox) : "(unknown)";
          const savedPath = j && j.path ? String(j.path) : "content/processed-content/";
          const abs = j && j.savedAbsPath ? String(j.savedAbsPath) : "";
          setStatus(
            `Crop saved. Wrote ${savedPath}. Saved viewBox: ${savedVb}. ${abs ? "Abs: " + abs + ". " : ""}${marksNote.trim()}`
          );
        }
      } catch (e) {
        setStatus(
     ...
     */
    function bbSyncUploadMarksChip() {
      try {
        const chip = document.querySelector(".bb-threshold-chip");
        if (chip) {
    ...
    function bbApplyUploadedAssetToMarks(opts) {
      ...
      const preserveSmartSizeCache = !!o.preserveSmartSizeCache;
  ...
  function bbSetSmartVars(el, result) {
    ...
    try {
      bbPersistLastSmartSize(el.dataset.brand, result);
    } catch {}
  }

  function bbPersistLastSmartSize(brand, result) {
    ...
    try {
      localStorage.setItem(
        `${BB_SMART_LAST_APPLIED_PREFIX}${b}`,
        JSON.stringify({
          scale: result.scale,
          offsetX: result.offsetX,
          offsetY: result.offsetY,
          sizeMul: result.sizeMul,
          maxWMul: result.maxWMul,
          paddingBottomPx: result.paddingBottomPx,
          __bbSource: result.__bbSource,
        })
      );
    } catch {}
  }

  function bbLoadLastSmartSize(brand) {
    const b = String(brand || "")
      .trim()
      .toLowerCase();
    if (!b) return null;
    try {
      const raw = localStorage.getItem(`${BB_SMART_LAST_APPLIED_PREFIX}${b}`);
  ...
  function bbFindLooseSmartSizeCache(brand, svgKey, baiForRules, goldenKey) {
    const brandKey = String(brand || "")
      .trim()
      .toLowerCase();
    if (!brandKey) return null;
    const head = `${BB_SMART_SIZE_CACHE_PREFIX}${brandKey}|`;
    const svgPart = `|svg:${svgKey}|`;
    const baiPart = `|bai:${baiForRules ? 1 : 0}|`;
    const gPart = goldenKey && goldenKey !== "none" ? `|g:${goldenKey}|` : "";
    try {
      for (let i = 0; i < localStorage.length; i++) {
        const k = localStorage.key(i);
        if (!k || !k.startsWith(head)) continue;
        if (!k.includes(svgPart) || !k.includes(baiPart)) continue;
        if (gPart && !k.includes(gPart)) continue;
  ...
  async function bbApplySmartSizing(el) {
    ...
    if (!bbForceSmartFetch) {
      ...
      const lastApplied = bbLoadLastSmartSize(brandKey0);
      if (lastApplied && typeof lastApplied.scale === "number") {
        bbSmartMemoryCache.set(key, lastApplied);
        bbApplyUserRulesToEl(el);
        bbRecordSmartSizeMeta(lastApplied, brand, true);
        bbSetSmartVars(el, lastApplied);
        try {
          localStorage.setItem(key, JSON.stringify(lastApplied));
        } catch {}
        return lastApplied;
      }

      const loose = bbFindLooseSmartSizeCache(brandKey0, svgKey, baiForRules, goldenKey);
      if (loose) {
        bbSmartMemoryCache.set(key, loose);
        bbApplyUserRulesToEl(el);
        bbRecordSmartSizeMeta(loose, brand, true);
        bbSetSmartVars(el, loose);
        try {
          localStorage.setItem(key, JSON.stringify(loose));
        } catch {}
        return loose;
      }
    }
  ...
  function openModalForBrandMark(el) {
    ...
    try {
      ...
      const mobileSheetClose = popup.querySelector(".popup-mobile-sheet-close");
      if (mobileSheetClose) {
        mobileSheetClose.classList.remove("bb-mobile-sheet-close-visible");
        window.clearTimeout(popup.__bbMobileSheetCloseTimer);
        popup.__bbMobileSheetCloseTimer = window.setTimeout(() => {
          try {
            if (!popup.classList.contains("popup-visible")) return;
            mobileSheetClose.classList.add("bb-mobile-sheet-close-visible");
          } catch {}
        }, 2000);
      }
  ...
  function bbGetStandaloneInTextMarks() {
    return Array.from(
      document.querySelectorAll(
        ".bb-premium-wrap .brandbased-dynamic-logo-slot, .bb-freemium-wrap .brandbased-dynamic-logo-slot"
      )
    ).filter((el) => !el.closest(".popup"));
  }

  /** Match Theme Styles simulated in-text mark (upload + short label "brand"). */
  function bbSyncStandaloneMarksToThemeSim() {
    let lightLu = "";
    let darkLu = "";
    try {
      lightLu = localStorage.getItem(BB_LU_LIGHT_LOGO_KEY) || "";
    ...
    if (!lightLu && !darkLu) return;
    bbPrepareStandaloneLogoCssOnly();
    if (!bbStandaloneMarksRevealed()) return;
    const scheme = bbResolveAppearanceScheme();
    bbPaintUploadMarksForScheme(scheme);
  }

  function bbRefreshInTextMarksSmartSizing(selector) {
    const marks = Array.from(document.querySelectorAll(selector)).filter(
      (el) => el.classList.contains("bb-enhanced") && !el.closest(".popup")
    );
  ...
  function bbRefreshStandaloneInTextSmartSizing() {
    bbRefreshInTextMarksSmartSizing(
      ".bb-premium-wrap .brandbased-dynamic-logo-slot, .bb-freemium-wrap .brandbased-dynamic-logo-slot"
    );
  }

  /** Theme Settings / Products preview: #bbThemeSimulatedHost in-text mark. */
  function bbRefreshThemeSimSmartSizing() {
    bbRefreshInTextMarksSmartSizing("#bbThemeSimulatedHost .brandbased-dynamic-logo-slot");
  }
  ...
  function bbGetGuidancePadTargetMarks() {
    ...
    document.querySelectorAll("#bbPreviewHost .brandbased-dynamic-logo-slot").forEach((el) => add(el));
    bbGetStandaloneInTextMarks().forEach((el) => add(el));
    return out;
  }
  ...
  /** Slider values from localStorage when placement panel is not on the page (standalone modals). */
  ...
  function bbGetPreviewPlacementMarks() {
    ...
    const seen = new Set();
    const add = (el) => {
      if (!el || seen.has(el) || el.closest?.(".popup")) return;
  ...
  function bbInitStandaloneAppearanceToggle() {
    const lightBtn = document.getElementById("bbStandaloneLight");
    const darkBtn = document.getElementById("bbStandaloneDark");
    if (!lightBtn && !darkBtn) return;
    const apply = (scheme) => {
      const s = scheme === "dark" ? "dark" : "light";
      try {
        document.documentElement.setAttribute("data-bb-scheme", s);
        document.body.classList.toggle("light-mode", s === "light");
        document.body.classList.toggle("dark-mode", s === "dark");
        sessionStorage.setItem(BB_STANDALONE_APPEARANCE_KEY, s);
      } catch (_e) {}
      lightBtn?.setAttribute("aria-pressed", s === "light" ? "true" : "false");
      darkBtn?.setAttribute("aria-pressed", s === "dark" ? "true" : "false");
      try {
        bbPrepareStandaloneLogoCssOnly();
        if (bbStandaloneMarksRevealed()) {
          bbPaintUploadMarksForScheme(s);
        }
        try {
          if (window.BBThemeConsoleSync && typeof window.BBThemeConsoleSync.sync === "function") {
            window.BBThemeConsoleSync.sync();
    ...
    let saved = "light";
    try {
      saved = sessionStorage.getItem(BB_STANDALONE_APPEARANCE_KEY) || "light";
    } catch (_e) {}
    apply(saved);
  ...
  function bbPrepareStandaloneLogoCssOnly() {
    let light = "";
    let dark = "";
    try {
      light = localStorage.getItem(BB_LU_LIGHT_LOGO_KEY) || "";
    ...
    if (light && dark) {
      const sig = `${light.length}|${dark.length}|${light.slice(0, 48)}|${dark.slice(0, 48)}`;
      if (sig !== bbLuDualLogoSig) bbApplyDualLuLogosToCss(light, dark);
    } else if (light || dark) {
      const pick = scheme === "dark" ? dark || light : light || dark;
      if (pick) bbApplyLogoDataUrl(pick, { persistAssetLab: false });
    } else {
      try {
        const cached = localStorage.getItem(BB_ASSET_LAB_STORAGE_KEY) || "";
        if (cached && /<\s*svg\b/i.test(cached)) bbApplyUploadedSvgToCss(cached);
      } catch (_e) {}
    }
  }

  function bbBootstrapStandaloneInTextMarks() {
    const standaloneSel =
      ".bb-premium-wrap .brandbased-dynamic-logo-slot, .bb-freemium-wrap .brandbased-dynamic-logo-slot";
    bbPrepareStandaloneLogoCssOnly();
    if (bbHasAppearanceAwareLogos()) {
      document.querySelectorAll(standaloneSel).forEach((el) => {
        if (el.closest(".popup")) return;
        el.dataset.brand = "upload";
        bbEnsureUploadMarkBrandToken(el);
      });
  ...
  function enhanceBrandMarks() {
    const isStandalone = bbIsStandaloneDemoPage();
    ...
    document.querySelectorAll(".brandbased-dynamic-logo-slot").forEach((el) => {
      ...
      if (!brand && el.classList.contains("brandbased")) {
        brand = "brandbased";
        el.dataset.brand = "brandbased";
      }
      if (!brand) return;
    ...
    try {
      ...
      if ((lightLu || darkLu) && !isStandalone) {
        bbPaintUploadMarksForScheme(bbResolveAppearanceScheme());
      }
    } catch (_e) {}
  }
  ...
  popup.innerHTML = `
    <div class="bb-popup-glass-base" aria-hidden="true"></div>
    <div class="bb-popup-theme-tint" aria-hidden="true"></div>
    <div class="bb-popup-theme-stack" aria-hidden="true">
      <div class="blur-bg"></div>
      <video class="bb-theme-video-bg" muted loop playsinline preload="metadata" aria-hidden="true"></video>
      <div class="bb-theme-logo-blend" aria-hidden="true"></div>
    </div>
    <div class="lock-icon-bb-logo" aria-hidden="true"></div>
    <button type="button" class="popup-mobile-sheet-close" aria-label="Close popup">
  ...
  const lockIconHost = popup.querySelector(".lock-icon-bb-logo");
  const token = bbBrandClass(brandSlug);
  if (token && brandIconEl) {
    brandIconEl.classList.add(token);
  }
  if (token && lockIconHost) {
    lockIconHost.classList.add(token);
  }

  // Premium standalone: always show the BrandBased "B" mark in the lock corner (inline SVG, recolorable).
  try {
    if (lockIconHost) {
      lockIconHost.querySelectorAll("svg.bb-lock-logo-inline").forEach((n) => n.remove());
  ...
  // Keep gallery dots tied to the real media count:
  // - 0/1 slide: no dots
  // - 2+ slides: Swiper renders exactly one dot per slide
  // This mirrors the product/theme page behavior where the gallery can
  // be rebuilt after media changes.
  const bbSyncGalleryDots = () => {
    const syncOne = (swiper, root) => {
      if (!root) return;
      const paginationEl = root.querySelector(".swiper-pagination");
      const slideCount = Array.from(root.querySelectorAll(".swiper-wrapper > .swiper-slide"))
        .filter((slide) => !slide.classList.contains("swiper-slide-duplicate"))
        .length;
      const shouldShow = slideCount > 1;
      if (paginationEl) {
        paginationEl.style.display = shouldShow ? "" : "none";
        if (shouldShow) {
          // .swiper-pagination has `width: auto !important` so without
          // dynamicBullets the default `left: 0` would park it on the
          // left edge. Re-centre it horizontally.
      ...
      if (!swiper) return;
      try { swiper.updateSlides?.(); } catch {}
      try { swiper.update?.(); } catch {}
      if (shouldShow) {
        try { swiper.pagination?.render?.(); } catch {}
        try { swiper.pagination?.update?.(); } catch {}
      }
    };
    syncOne(thumbSwiper, popup.querySelector(".swiper-thumb"));
    syncOne(sliderCloseUpSwiper, popup.querySelector(".slider-close-up-swiper"));
  };
  bbSyncGalleryDots();
  try {
    const observeGallery = (root) => {
      const wrap = root?.querySelector?.(".swiper-wrapper");
      if (!wrap) return;
      const mo = new MutationObserver(bbSyncGalleryDots);
      mo.observe(wrap, { childList: true });
    };
    observeGallery(popup.querySelector(".swiper-thumb"));
    observeGallery(popup.querySelector(".slider-close-up-swiper"));
  } catch {}
  ...
  const bbSetZoomOpen = (open) => {
    popup.classList.toggle("bb-zoom-open", !!open);
  };

  let bbCloseUpHome = null;

  const bbShouldPinCloseUpToPopup = () => {
    const w = window.innerWidth;
    return w >= 841 && w <= 980;
  };

  const bbMountCloseUpToPopup = () => {
    if (!bbShouldPinCloseUpToPopup() || !sliderCloseUp || !popup) return;
    if (sliderCloseUp.parentElement === popup) {
      sliderCloseUp.classList.add("bb-closeup-pinned-popup");
      return;
    }
    const productEl = popup.querySelector(".product-element");
    if (!productEl) return;
    bbCloseUpHome = { parent: productEl, next: sliderCloseUp.nextSibling };
    popup.appendChild(sliderCloseUp);
    sliderCloseUp.classList.add("bb-closeup-pinned-popup");
  };

  const bbRestoreCloseUpHome = () => {
    if (!sliderCloseUp) return;
    sliderCloseUp.classList.remove("bb-closeup-pinned-popup");
    if (!bbCloseUpHome?.parent) return;
    const { parent, next } = bbCloseUpHome;
    try {
      if (next && next.parentElement === parent) parent.insertBefore(sliderCloseUp, next);
      else parent.appendChild(sliderCloseUp);
    } catch {}
    bbCloseUpHome = null;
  };

  const bbDismissCloseUp = () => {
    if (!sliderCloseUp) return;
    sliderCloseUp.style.display = "none";
    bbSetZoomOpen(false);
    bbRestoreCloseUpHome();
  };

  // Close-up X: desktop = fixed inset; mobile = same screen position as .popup-mobile-sheet-close (rect sync).
  const bbPositionCloseUpX = () => {
    if (!sliderCloseUp || !closeBtn) return;

    const mobileSheetClose = popup.querySelector(".popup-mobile-sheet-close");
    const msStyles = mobileSheetClose ? window.getComputedStyle(mobileSheetClose) : null;
    const mobileSheetCloseActive =
      mobileSheetClose &&
      msStyles &&
      msStyles.display !== "none" &&
      msStyles.visibility !== "hidden" &&
      Number.parseFloat(msStyles.opacity || "1") > 0;

    const closeUpOpen = window.getComputedStyle(sliderCloseUp).display === "flex";

    if (
      closeUpOpen &&
      mobileSheetCloseActive &&
      popup.classList.contains("popup-visible")
    ) {
      const mr = mobileSheetClose.getBoundingClientRect();
      const sr = sliderCloseUp.getBoundingClientRect();
      closeBtn.style.left = "auto";
      closeBtn.style.top = `${mr.top - sr.top}px`;
      closeBtn.style.right = `${sr.right - mr.right}px`;
    ...
    // Desktop / tablet: anchor the close-up X to the swiper's TOP-RIGHT
    // corner so it always tracks the gallery's right edge regardless of
    // viewport width (the overlay is full-screen but the swiper is
    // 79–80vw, centred — fixed insets from the overlay placed the X out
    // in the gutter on mid-range widths).
    const swiperEl =
      sliderCloseUp.querySelector(".swiper.slider-close-up-swiper") ||
      sliderCloseUp.querySelector(".swiper");
    const inset = 8;
    if (swiperEl) {
      const sr = sliderCloseUp.getBoundingClientRect();
      const wr = swiperEl.getBoundingClientRect();
      closeBtn.style.top   = `${wr.top - sr.top + inset}px`;
      closeBtn.style.right = `${sr.right - wr.right + inset}px`;
    } else {
      closeBtn.style.top   = `${inset}px`;
  ...
  const bbSyncCloseUpPin = () => {
    if (bbShouldPinCloseUpToPopup()) bbMountCloseUpToPopup();
    else bbRestoreCloseUpHome();
  };

  const bbOnCloseUpLayoutChange = () => {
    if (!sliderCloseUp || window.getComputedStyle(sliderCloseUp).display !== "flex") return;
    bbSyncCloseUpPin();
    try {
      sliderCloseUpSwiper.update();
    } catch {}
    bbPositionCloseUpX();
  };

  const bbCloseUpSwiperEl = () =>
    sliderCloseUp?.querySelector(".swiper.slider-close-up-swiper") ||
    sliderCloseUp?.querySelector(".swiper") ||
    null;

  const bbCloseUpSwiperHasLayout = () => {
    const el = bbCloseUpSwiperEl();
    if (!el) return true;
    const wr = el.getBoundingClientRect();
    return wr.width > 1 && wr.height > 1;
  };

  /* First open: Swiper often has 0×0 (or stale) size for a few frames on wide
     viewports — retry until layout matches what resize already sees. */
  const bbPositionCloseUpXWhenReady = (triesLeft = 16) => {
    if (!sliderCloseUp || window.getComputedStyle(sliderCloseUp).display !== "flex") return;
    if (!bbCloseUpSwiperHasLayout()) {
      if (triesLeft > 0) {
        try {
          requestAnimationFrame(() => bbPositionCloseUpXWhenReady(triesLeft - 1));
        } catch {}
      }
      return;
    }
    bbOnCloseUpLayoutChange();
  };

  const bbPositionCloseUpXAfterLayout = () => {
    bbPositionCloseUpXWhenReady();
    try {
      sliderCloseUpSwiper.once("resize", () => bbOnCloseUpLayoutChange());
    } catch {}
    try {
      window.setTimeout(() => bbPositionCloseUpXWhenReady(4), 0);
    } catch {}
  };
  try {
    window.addEventListener("resize", bbOnCloseUpLayoutChange, { passive: true });
    window.addEventListener("orientationchange", bbOnCloseUpLayoutChange, { passive: true });
  } catch {}
  ...
  // Close close-up view
  closeBtn.onclick = () => { bbDismissCloseUp(); };
  sliderCloseUp.addEventListener('click', e => {
    if (e.target === sliderCloseUp) bbDismissCloseUp();
  });
  ...
  // Double-activate on close-up media = same as "+" (Swiper default preventClicks blocks native dblclick).
  const closeUpSwiperEl = popup.querySelector(".slider-close-up-swiper");
  const bbCloseUpDoubleZoom = (e) => {
    const m = e.target?.closest?.("img,video");
    if (!m) return;
    const activeSlide = sliderCloseUpSwiper.slides?.[sliderCloseUpSwiper.activeIndex];
    if (!activeSlide || !activeSlide.contains(m)) return;
    try {
      e.preventDefault();
    } catch {}
    try {
  ...
  closeUpSwiperEl?.addEventListener("dblclick", bbCloseUpDoubleZoom);
  let _bbCloseUpLastTouchEnd = 0;
  closeUpSwiperEl?.addEventListener(
    "touchend",
    (e) => {
      if (e.changedTouches?.length !== 1) {
        _bbCloseUpLastTouchEnd = 0;
        return;
      }
      const m = e.target?.closest?.("IMG,VIDEO");
      if (!m) return;
      ...
      if (!activeSlide || !activeSlide.contains(m)) return;
      const now = Date.now();
      const dt = _bbCloseUpLastTouchEnd ? now - _bbCloseUpLastTouchEnd : 99999;
      if (_bbCloseUpLastTouchEnd && dt > 40 && dt < 500) {
        try {
          e.preventDefault();
        } catch {}
        bumpZoom();
  ...
  const openCloseUpFromThumb = (index) => {
    bbSetZoomOpen(true);
    bbSyncCloseUpPin();
    ...
    bbPositionCloseUpXAfterLayout();
  };

  let _bbThumbSuppressClickUntil = 0;
  let _bbLastThumbTapAt = 0;
  popup.querySelectorAll(".swiper-thumb .swiper-slide").forEach((slide, index) => {
    slide.addEventListener("click", () => {
      if (Date.now() < _bbThumbSuppressClickUntil) return;
      openCloseUpFromThumb(index);
    });
    const thumbMedia = slide.querySelector("img,video");
    if (thumbMedia) {
      thumbMedia.addEventListener(
        "click",
        (e) => {
          if (e.detail !== 2) return;
          e.preventDefault();
          e.stopPropagation();
          _bbThumbSuppressClickUntil = Date.now() + 500;
          openCloseUpFromThumb(index);
          try {
            requestAnimationFrame(() => {
              requestAnimationFrame(() => bumpZoom());
            });
          } catch {}
        },
        true
...
/* Close-up X fade — do not resize .popup (theme layers must not change modal shape). */
...
const positionPopup = () => {
  if (window.innerWidth < 840) {
    ...
    closeIconDesktop?.addEventListener('click', closePopupUi);
    mobileSheetClose?.addEventListener('click', closePopupUi);

    let bbSheetTouch = null;
    const clearSheetTouch = () => {
      bbSheetTouch = null;
    };
    popup.addEventListener(
      ...
      (e) => {
        ...
        if (t.closest(".product-description") || t.closest(".product-description-holder")) return;
        if (t.closest(".selectors-row") || t.closest(".buy-now-row")) return;
        if (t.closest("a[href], button, input, select, textarea")) return;
        bbSheetTouch = { y0: e.touches[0].clientY, x0: e.touches[0].clientX };
      },
      { passive: true }
    );
    popup.addEventListener(
      "touchend",
      (e) => {
        if (!bbSheetTouch) return;
        const y1 = e.changedTouches[0].clientY;
        const x1 = e.changedTouches[0].clientX;
        const dy = y1 - bbSheetTouch.y0;
        const dx = x1 - bbSheetTouch.x0;
        clearSheetTouch();
        if (dy > 72 && Math.abs(dx) < 100) closePopupUi();
      },
      { passive: true }
    );
    popup.addEventListener("touchcancel", clearSheetTouch, { passive: true });
    ...
    popupHandle?.addEventListener("click", () => {
      if (canCloseMobile) closePopupUi();
    });
    ...
    popupHandle?.addEventListener("touchend", (e) => {
      ...
      if (canCloseMobile && touchEndY - touchStartY > 40) closePopupUi();
    });
...
const _bbInitDelayMs = bbIsStandaloneDemoPage() ? 1000 : 2000;
setTimeout(() => {
  if (!brandbasedEnhancementEnabled) return;
  if (bbIsStandaloneDemoPage()) {
    bbBootstrapStandaloneInTextMarks();
    try {
      if (bbIsBrandAiSmartOn()) bbApplyGuidanceLrPadFromText();
    } catch {}
    return;
  }
  enhanceBrandMarks();
