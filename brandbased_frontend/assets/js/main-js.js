...
window.addEventListener('load', () => {
  ...
  setTimeout(() => {
    ...
    // Hide splash after fade transition
    setTimeout(() => {
      splash.style.display = 'none';
      appInterface.style.display = 'block';
    }, 300);
  }, 2000); // increase/decrease this value
});
...
function OpenLoginForm() {
  ...
  const buttonsHide = document.getElementById("buttons-hide");
  const slogan = document.getElementById("slogan");
  const footerTag = document.getElementById("footer-tag");

  if (loginForm) loginForm.style.display = "block";
  if (joinForm) joinForm.style.display = "none";
  if (buttonsHide) buttonsHide.style.display = "none";
...
function CloseLoginForm() {
  ...
  if (buttonsHide) buttonsHide.style.display = "flex";
  if (slogan) slogan.style.display = "block";
  if (footerTag) footerTag.style.display = "block";
}
...
function enableBrandBasedEnhancement() {
  ...
  const replacementHTML = `<img class="bb-replace" src="assets/images/BB-TYPEFACE-LOGO-white.svg" alt="BrandBased" style="height: 1em; vertical-align: top;">`;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (!footerNav || !openFooterNav || !closeMobileNav) return;

  openFooterNav.addEventListener('click', () => {
    footerNav.classList.add('active');
    closeMobileNav.classList.remove('minus');
  });
